...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['media_file'])) {
    ...
    // 调试信息：记录检测到的路径
    error_log("Upload: Subdomain URL: $subdomainUrl");
    error_log("Upload: Upload directory: $uploadDir");
    ...
    if (!isset($error)) {
        ...
        if ($file['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($mimeType, $allowedMimes)) {
                $error = "文件MIME类型验证失败！";
                error_log("Upload error: Invalid MIME type $mimeType for file " . $file['name']);
            } else {
                // 生成新文件名
                ...
                if (move_uploaded_file($file['tmp_name'], $targetPath)) {
                    ...
                    error_log("Upload success: Photo #{$photoNumber} saved to $targetPath, URL: " . $subdomainUrl . $newFileName);
                    ...
                    error_log("Upload error: Failed to move file to $targetPath");
                }
            }
        }
    }
