...
// 读取店面会话，限制为本页支持的类型
$_sessionStore = strtolower($_SESSION['store'] ?? 'j3');
if (!in_array($_sessionStore, ['j1', 'j2', 'j3', 'central'])) {
    $_sessionStore = 'j3';
}
?>
<!DOCTYPE html>
...
<body>
    ...
    <script>
        ...
        function initApp() {
            ...
            // API健康检查（仅日志）
            fetch(`${API_BASE_URL}?action=ping`).then(r=>r.text()).then(t=>{try{console.log('API ping:',JSON.parse(t));}catch(e){console.warn('API ping raw:',t);}}).catch(e=>console.error('API ping failed:',e));
        ...
        async function apiCall(endpoint, options = {}) {
            try {
                ...
                try {
                    ...
                    console.error('API invalid JSON raw:', text);
    ...
    <script>
        ...
        async function selectComboboxOption(optionElement, input) {
            ...
            if (type === 'code') {
                ...
                if (result) {
                    ...
                    let relatedInput = rowEl ? rowEl.querySelector('.combobox-input[data-type="product"]') : null;
                    if (!relatedInput) {
                        ...
                        if (isNewRow) {
                            ...
                            relatedInputId = rowIdMatch ? `${rowIdMatch[1]}-product_name-input` : 'new-product_name-input';
                        } else {
                            relatedInputId = `combo-product_name-${recordId}-input`;
                        }
                        relatedInput = document.getElementById(relatedInputId);
                    }
                    if (relatedInput) {
                ...
                if (result) {
                    const { product_code, specification, supplier, category } = result;
                    const rowEl = input.closest('tr');
                    let relatedInput = rowEl ? rowEl.querySelector('.combobox-input[data-type="code"]') : null;
                    if (!relatedInput) {
                        const containerId = input.closest('.combobox-container').id || '';
                        const isNewRow = containerId.includes('new-');
                        let relatedInputId;
                        if (isNewRow) {
                            const rowIdMatch = containerId.match(/^(new-\d+)-/);
                            relatedInputId = rowIdMatch ? `${rowIdMatch[1]}-code_number-input` : 'new-code_number-input';
                        } else {
        ...
        // 价格选项更新（J3移动版不使用价格，留空实现以避免报错）
        function updatePriceOptions(container, productName) {
            // no-op: 本页面已移除价格/总价逻辑
            return;
        }
    ...
    <script>
        ...
        // loadProductPricesWithStock 函数已删除（不连接数据库）
        async function loadProductPricesWithStock_DELETED(productName, selectElementId, currentPrice = '', requiredQty = 0) {
            try {
                const result = await apiCall(`?action=product_prices_with_stock&product_name=${encodeURIComponent(productName)}&required_qty=${requiredQty}`);
                const selectElement = document.getElementById(selectElementId);
