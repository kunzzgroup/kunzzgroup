...
function handleGet() {
    ...
    switch ($action) {
        ...
        case 'product_stock':
            // 按货品总可用库存（与总库存一致：合并 j3stockeditmobile_data + j3stockedit_data）
            $productName = $_GET['product_name'] ?? null;
            $codeNumber = $_GET['code_number'] ?? null;
            if (!$productName) {
                sendResponse(false, "缺少产品名称参数");
            ...
            try {
                $sql = "SELECT SUM(in_quantity) - SUM(out_quantity) as available_stock
                        FROM (
                            SELECT in_quantity, out_quantity FROM j3stockeditmobile_data
                ...
                $sql .= " UNION ALL
                            SELECT in_quantity, out_quantity FROM j3stockedit_data
                            WHERE product_name = ? AND product_name IS NOT NULL AND product_name != ''";
                $params[] = $productName;
                if ($codeNumber !== null && $codeNumber !== '') {
...
// 同步数据到 j3stockedit_data 表（出库时按价格从高到低分配：先扣高单价，再扣下一档）
