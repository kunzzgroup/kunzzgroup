...
if (!in_array('KH', $user_branches) && !in_array('J3', $user_branches)) {
    http_response_code(403);
    echo json_encode(["success" => false, "message" => "Access denied: Unauthorized branch. (User Branch: " . ($_SESSION['branch'] ?? '') . ", Required: J3 or KH)"]);
    exit;
}
...
function ensureTables(PDO $pdo) {
    ...
    // 更新表结构以包含缺失字段
    try {
        $pdo->exec("ALTER TABLE `j3stockeditmobile_data` ADD COLUMN `specification` varchar(255) DEFAULT NULL AFTER `code_number` ");
    } catch (PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE `j3stockeditmobile_data` ADD COLUMN `type` varchar(100) DEFAULT NULL AFTER `specification` ");
    } catch (PDOException $e) {}
    ...
    // 添加被回收站软删除的支持
    try {
        $pdo->exec("ALTER TABLE `j3stockeditmobile_data` ADD COLUMN `deleted_at` timestamp NULL DEFAULT NULL");
        $pdo->exec("ALTER TABLE `j3stockeditmobile_data` ADD COLUMN `deleted_by` varchar(100) DEFAULT NULL");
    } catch (PDOException $e) {}
    ...
    // 更新索引和字段以支持规格
    try {
        $pdo->exec("ALTER TABLE `j3stocklist_total` ADD COLUMN `specification` varchar(255) DEFAULT NULL AFTER `code_number` ");
    } catch (PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE `j3stocklist_total` DROP INDEX `unique_product` ");
    } catch (PDOException $e) {}
    try {
        $pdo->exec("ALTER TABLE `j3stocklist_total` ADD UNIQUE KEY `unique_product_spec` (`product_name`, `code_number`, `specification`) ");
    } catch (PDOException $e) {}
...
function handleGet() {
    ...
    switch ($action) {
        case 'list':
            ...
            $sql = "SELECT * FROM j3stockeditmobile_data WHERE deleted_at IS NULL";
            $params = [];
...
function updateStocklistTotal($productName, $codeNumber, $inQty, $outQty, $isAdd = true, $specification = null) {
    ...
    try {
        ...
        $sql = "SELECT * FROM j3stocklist_total WHERE product_name = ? AND code_number = ? ";
        ...
        if ($existing) {
            ...
            // 创建新记录
            if ($netQty > 0 || $isAdd) {
                $insertStmt = $pdo->prepare("INSERT INTO j3stocklist_total (product_name, code_number, specification, total_qty) VALUES (?, ?, ?, ?)");
...
function handleBatchSave() {
    ...
    try {
        ...
        validateBatchOutboundPriceTiers($pdo, 'j3stockedit_data', $rows);
        ...
        foreach ($rows as $index => $row) {
            ...
            if (hasRecentDuplicateMobileBatchRow($pdo, 'j3stockeditmobile_data', $documentDate, $row)) {
            ...
            $syncOk = syncToJ3StockEditData($pdo, $syncData, 'insert');
            if ($syncOk === false) {
