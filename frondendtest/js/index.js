document.addEventListener('DOMContentLoaded', function() {
    ...
    // ==========================================
    // 3. Page Indicator Functionality
    // ==========================================
...
// ==========================================
// 6. Window Load: Video & Image Animations
// ==========================================
window.addEventListener('load', () => {
    ...
    if (bgImage) {
        ...
        // 如果图片已经加载完成
        if (bgImage.complete) {
            bgImage.dispatchEvent(new Event('load')); // 或者是直接调用 triggerAnimations()
            triggerAnimations();
        }
    }
    ...
    setTimeout(() => {
        const homeSection = document.querySelector('.home');
        if (homeSection && !homeSection.classList.contains('gradient-loaded')) {
            triggerAnimations();
        }
    }, 500);
});
...
// ==========================================
// 7. Map Navigation (Global Function)
// ==========================================
function goToLocation() {
    const map = document.getElementById('custom-map');
    if (map) {
        // ⚠️ 这里请替换成你 My Maps 中标记具体地点的链接
