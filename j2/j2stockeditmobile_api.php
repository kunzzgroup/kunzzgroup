...
// 同步数据到 j2stockedit_data 表（出库时按价格从高到低分配：先扣高单价，再扣下一档）
function syncToJ2StockEditData($pdo, $data, $operation = 'insert') {
    try {
        ...
        if ($operation === 'delete') {
            // 删除该条手机记录对应的所有 edit 行（可能有多条按价格分配的行）
        ...
        // insert 或 update：出库 > 0 时按价格从高到低分配写入多条；否则写一条
        if ($outQty <= 0 && $inQty <= 0) {
            return true;
        }

        if ($outQty > 0 && $inQty <= 0) {
            // 按价格从高到低查 j2stockedit_data 可用库存，分配出库
            ...
            if ($remaining > 0) {
                // 无足够按价格分档的库存时，退化为单条写入（价格从 stock_data 取）
