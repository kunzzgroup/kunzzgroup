...
function ensureTables(PDO $pdo)
{
    ...
    // 添加被回收站软删除的支持
    try {
        $pdo->exec("ALTER TABLE `j2stockeditmobile_data` ADD COLUMN `deleted_at` timestamp NULL DEFAULT NULL");
        $pdo->exec("ALTER TABLE `j2stockeditmobile_data` ADD COLUMN `deleted_by` varchar(100) DEFAULT NULL");
    }
    catch (PDOException $e) {
    }
...
function handleGet()
{
    ...
    switch ($action) {
        case 'list':
            ...
            $sql = "SELECT * FROM j2stockeditmobile_data WHERE deleted_at IS NULL";
            $params = [];
...
function syncToJ2StockEditData($pdo, $data, $operation = 'insert')
{
    try {
        ...
        $matchInfo = null;
        $qStmt = $pdo->prepare("SELECT specification, COALESCE(price, 0) as price, type FROM j2stockedit_data
        ...
        error_log("syncToJ2StockEditData: product={$productName}, out={$outQty}, price={$price}");
        return true;
