...
.back-btn:hover { background: #ea6b0a; color: #fff; transform: translateY(-1px); }
...
.left-card::-webkit-scrollbar { width: 6px; }
.left-card::-webkit-scrollbar-track { background: #f1f1f1; border-radius: 4px; }
.left-card::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
.left-card::-webkit-scrollbar-thumb:hover { background: #94a3b8; }

.form-col > .form-section:last-child { flex: 1; min-height: 0; }
...
.form-section-header,
.form-section-header-bank { flex-shrink: 0; }
...
.perm-tree-container::-webkit-scrollbar { width: 5px; }
.perm-tree-container::-webkit-scrollbar-thumb { background: #f0a060; border-radius: 3px; }
.perm-tree-container::-webkit-scrollbar-track { background: #f9f9f9; }
...
.perm-arrow-store { font-size: 10px; color: #f97316; transition: transform 0.2s; }
.perm-arrow-store.open { transform: rotate(90deg); }
.perm-store-content { display: none; padding: 8px 8px 4px 20px; }
.perm-store-content.open { display: block; }

/* 响应式 */
@media (max-width: 1200px) {
    .form-scroll-area { overflow-y: auto; align-items: flex-start; }
    #addUserForm { grid-template-columns: 1fr; max-width: 100%; height: auto; }
    .form-col { height: auto; overflow: visible; }
    .left-card { flex: none; height: auto; min-height: unset; overflow-y: visible; }
    .form-col > .form-section:last-child,
    .form-col > .editUserPermLayout:last-child { flex: none; overflow: visible; }
    .perm-layout-container { grid-template-columns: 1fr; }
    .perm-tree-container { max-height: 400px !important; border-right: none; border-bottom: 1px solid #e5e7eb; }
    .perm-detail-card { max-height: 400px !important; }
}
@media (max-width: 768px) {
    .page-header-bar { padding: 12px 14px; }
    .page-header-bar h1 { font-size: 16px; }
    .back-btn { padding: 6px 12px; font-size: 13px; }
    .form-scroll-area { padding: 10px; }
    .form-grid-3, .form-grid-2, .form-row-2col { grid-template-columns: 1fr; gap: 12px; }
    .form-section-content { padding: 14px; }
    .form-section-header, .form-section-header-bank { padding: 10px 14px; font-size: 13px; }
    .page-action-bar { padding: 12px 16px; flex-direction: column-reverse; gap: 8px; }
    .btn-save, .btn-back-action { width: 100%; justify-content: center; padding: 12px; font-size: 15px; }
    #toast-container { bottom: 110px; left: 50%; right: auto; transform: translateX(-50%); align-items: center; width: 90vw; }
}

/* Cards */
.form-section { background: #fff; border: 1px solid #e5e7eb; border-radius: 10px; overflow: hidden; box-shadow: 0 1px 3px rgba(0,0,0,.04); }
.form-section-header { background: #f97316; padding: 13px 20px; font-size: 15px; font-weight: 600; color: #fff; }
.form-section-header-bank { background: #f97316; padding: 13px 20px; font-size: 15px; font-weight: 600; color: #fff; margin-top: 10px; }
...
.form-group label { font-size: 13px; font-weight: 600; color: #374151; margin-bottom: 6px; min-height: 20px; display: flex; align-items: center; }
.form-group input,
.form-group select,
.form-group textarea {
    padding: 9px 12px;
...
.form-group textarea:focus { outline: none; border-color: #f97316; box-shadow: 0 0 0 3px rgba(249,115,22,.12); }
...
#toast-container {
    ...
    align-items: flex-end; gap: 8px; pointer-events: none; max-width: 340px;
}
.toast {
    display: inline-flex; align-items: center; gap: 10px;
    padding: 12px 20px; border-radius: 10px; font-size: 13.5px; font-weight: 600;
    box-shadow: 0 6px 24px rgba(0,0,0,.16); pointer-events: all;
    animation: toastIn .3s cubic-bezier(.21,1.02,.73,1) forwards; width: 100%;
}
...
.required-mark { color: #ef4444; font-size: 14px; margin-left: 2px; line-height: 1; }
    </style>
</head>
<body>
                        ...
                        <div class="form-section-content">
                            <div class="perm-layout-container">
                                ...
                                <div class="perm-detail-card">
                                    ...
                                    </div><!-- /perm-detail-content -->
                                </div><!-- /perm-detail-card -->

                            </div><!-- /perm-layout-container -->
    ...
    <script>
        ...
        function initPermissionTreeEvents(container) {
            ...
            // 二级 checkbox → 反向联动一级（支持 indeterminate 半选）
            container.querySelectorAll('.perm-l2-check').forEach(cb => {
                cb.addEventListener('change', function () {
                    ...
                    l1.checked = checked > 0;
                    l1.indeterminate = checked > 0 && checked < siblings.length;
                });
            });

            // 详细配置链接 → 显示右侧面板，并自动展开父级
            container.querySelectorAll('.perm-cfg-link').forEach(link => {
                link.addEventListener('click', function (e) {
                    e.stopPropagation();
                    showDetailPanel(container, this.dataset.target);
                    const l1c = this.closest('.perm-level-2-item')?.closest('.perm-level-2-container');
                    if (l1c && !l1c.classList.contains('open')) {
                        l1c.classList.add('open');
                        const perm = l1c.dataset.parent;
            ...
            container.querySelectorAll('.perm-store-item > .perm-checkbox-label').forEach(label => {
                label.addEventListener('click', function () {
                    const content = this.closest('.perm-store-item').querySelector('.perm-store-content');
            ...
            container.querySelectorAll('input[type="checkbox"]').forEach(cb => {
                cb.addEventListener('change', () => {
                    container.closest('.editUserPermLayout')?.querySelector('.perm-warning')
        ...
        function showDetailPanel(container, target) {
            ...
            detail.querySelector(`.perm-level-3-panel[data-for="${target}"]`)?.classList.add('active');
        }
        ...
        function showInlineMessage(msg, type = 'error') {
            ...
            setTimeout(() => {
                if (toast.parentNode === container) {
                    ...
                    setTimeout(() => toast.parentNode === container && toast.remove(), 320);
                }
            }, type === 'success' ? 3000 : 4500);
        }
        ...
        async function addNewUserAndRedirect() {
            ...
            const userData = {};
            for (let [k, v] of formData.entries()) userData[k] = v.trim();
            ...
            if (hasError) { showInlineMessage('请填写所有必填项（*）', 'error'); return; }
            ...
            try {
                ...
                if (result.success) {
                    ...
                    msg += result.data?.email_sent
                        ? ` 登录信息已发送至 ${result.data.email}`
                        : ` 临时密码：${result.data?.default_password || ''}`;
