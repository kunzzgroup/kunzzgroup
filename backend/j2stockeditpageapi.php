...
function handleDelete()
{
    ...
    try {
        ...
        foreach ($targetIds as $currentId) {
            ...
            require_once __DIR__ . '/stock_inventory_guard.php';
            $guard = assertDeleteInWouldNotGoNegative($pdo, 'j2stockedit_data', $recordToDelete);
            if (!$guard['ok']) {
                $pdo->rollBack();
    ...
    try {
        ...
        foreach ($rows as $index => $row) {
            ...
            if (strtolower($targetSystem) === 'central') {
                ...
                $centralId = $pdo->lastInsertId();
                if ($centralId) {
                    $pdo->prepare("UPDATE j2stockedit_data SET main_record_id = ?, target_system = 'Central' WHERE id = ?")
