...
function aboutpage1upload_getReturnTo() {
...
function aboutpage1upload_redirectAfterAction($query = '') {
    $returnTo = aboutpage1upload_getReturnTo();
    if ($returnTo === 'v2') {
        $base = rtrim(str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? '/backend')), '/');
        header('Location: ' . $base . '/aboutpage1upload-v2' . $query);
        exit();
    }

    $self = basename($_SERVER['SCRIPT_NAME'] ?? 'aboutpage1upload.php');
    header('Location: ' . $self . $query);
    exit();
}

function aboutpage1upload_loadConfig() {
...
function aboutpage1upload_diskPath($webPath) {
    if (!$webPath) {
        return '';
    }
...
function aboutpage1upload_resolveMediaSrc($webPath) {
    if (!$webPath) {
        return '';
    }
...
function aboutpage1upload_mediaVersion($webPath) {
    $diskPath = aboutpage1upload_diskPath($webPath);
...
function aboutpage1upload_handlePost(&$success, &$error) {
    ...
    $mediaType = $_POST['media_type'] ?? 'about_background';
    ...
    aboutpage1upload_redirectAfterAction('?success=1&t=' . time());
}
