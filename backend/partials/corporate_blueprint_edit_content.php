<?php
$formActionUrl = $formActionUrl ?? 'corporate_blueprint_edit.php';
$pageUrl = $pageUrl ?? 'corporate_blueprint_edit.php';
$viewUrl = $viewUrl ?? 'corporate_blueprint.php';
$returnTo = $returnTo ?? '';
$success = $success ?? '';
$error = $error ?? '';
$companyOverview = $companyOverview ?? [];
$orgStructure = $orgStructure ?? ['ceo' => [], 'pa' => [], 'cLevel' => []];
$internalOrg = $internalOrg ?? ['departments' => []];
$timeline = $timeline ?? [];
$corporateCore = $corporateCore ?? ['mission' => '', 'vision' => '', 'culture' => [], 'values' => []];
$cultureExplanation = $cultureExplanation ?? [];
$valuesExplanation = $valuesExplanation ?? [];
$strategicObjectives = $strategicObjectives ?? [];
$requestMethod = $requestMethod ?? 'GET';
?>
<div class="container" data-corporate-blueprint-edit-root>
        <div class="header">
            <h1>企业蓝图管理</h1>
            <p>编辑企业蓝图数据和咨询信息</p>
        </div>
        ...
        <div class="content">
            ...
            <?php if ($requestMethod === 'POST' && empty($success) && empty($error)): ?>
                <div class="alert alert-error">表单已提交，但未检测到保存操作。请检查表单数据是否正确提交。POST 数据数量: <?php echo count($_POST); ?></div>
            <?php endif; ?>
            
            <form method="POST" action="<?php echo htmlspecialchars($formActionUrl, ENT_QUOTES, 'UTF-8'); ?>" id="corporate-form" onsubmit="return handleFormSubmit(event)">
...
Implementation Timeline Adherence (%)"><?php echo htmlspecialchars(implode("\n", $obj['dashboardMetrics'] ?? [])); ?></textarea>
                ...
                <div class="fixed-actions">
                    ...
                    <a href="<?php echo htmlspecialchars($viewUrl, ENT_QUOTES, 'UTF-8'); ?>" class="btn btn-secondary">返回查看</a>
                </div>
            </form>
        </div>
    </div>
<script>
window.clevelIndex = <?php echo count($orgStructure['cLevel'] ?? []); ?>;
window.deptIndex = <?php echo count($internalOrg['departments'] ?? []); ?>;
window.timelineIndex = <?php echo count($timeline); ?>;
window.cultureIndex = <?php echo count($corporateCore['culture'] ?? []); ?>;
window.valuesIndex = <?php echo count($corporateCore['values'] ?? []); ?>;
window.cultureExplanationIndex = <?php echo count($cultureExplanation); ?>;
window.valuesExplanationIndex = <?php echo count($valuesExplanation); ?>;
</script>
