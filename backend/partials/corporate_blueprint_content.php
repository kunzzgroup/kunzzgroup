...
$internalOrgChartData = $internalOrgChartData ?? [];
$allObjectives = $allObjectives ?? [];
$ultimateGoal = $ultimateGoal ?? '';
$strategyEndYear = $strategyEndYear ?? ((int) date('Y') + 5);
$logoPath = $logoPath ?? '../images/images/logo.png';
$logoFullPath = $logoFullPath ?? (dirname(__DIR__) . '/../images/images/logo.png');
?>
    <div class="main-content">
        <div class="main-container" data-corporate-blueprint-root>
            <!-- 页面标题 -->
            <div class="header">
            ...
            <?php if ($strategyData): ?>
                            ...
                            if (!empty($strategyData['timeline'])): 
                                ...
                                foreach ($strategyData['timeline'] as $index => $item):
                                    ...
                                    if ($t <= 0.5) {
                                        ...
                                        $point = corporate_blueprint_bezierQuad($t_curve, $p0_1, $p1_1, $p2_1);
                                    } else {
                                        // Second half: use second Bezier curve
                                        $t_curve = ($t - 0.5) * 2; // Map to 0-1 for second curve
                                        $point = corporate_blueprint_bezierQuad($t_curve, $p0_2, $p1_2, $p2_2);
                                    }
                ...
                <?php if (!empty($internalOrgChartData) && is_array($internalOrgChartData)): ?>
                <!-- 内部组织架构图 -->
                <div class="section">
                    <div class="orgchart-container-wrapper">
                ...
                <script>
                // Pass data from PHP (window.* so React v2 can re-inject fragment safely)
                window.strategiesData = <?php echo json_encode($allObjectives, JSON_UNESCAPED_UNICODE); ?>;
                window.orgData = <?php echo !empty($orgChartData) ? json_encode($orgChartData, JSON_UNESCAPED_UNICODE) : 'null'; ?>;
                window.internalOrgData = <?php echo !empty($internalOrgChartData) ? json_encode($internalOrgChartData, JSON_UNESCAPED_UNICODE) : 'null'; ?>;
                </script>
<?php else: ?>
                <!-- 如果没有JSON数据，显示错误信息 -->
                <div class="card">
