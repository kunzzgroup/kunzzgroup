    ...
    stockData.forEach(record => {
        ...
        row.innerHTML = `
                ...
                `<span>${record.product_name}</span>`
                    ...
                    <td>
                        ${isEditing ?
                `<input type="number" class="table-input" value="${record.in_quantity || ''}" min="0" step="0.001" onchange="updateField(${record.id}, 'in_quantity', this.value)" oninput="enforceQuantityMutex(this, this.closest('tr').querySelector('input[onchange*=\\'handleEditOutQuantityChange\\']'))" ${(parseFloat(record.out_quantity) || 0) > 0 ? 'disabled' : ''}>` :
                `<span>${formatNumber(record.in_quantity)}</span>`
            }
                    </td>
                    <td>
                        ${isEditing ?
                `<input type="number" class="table-input" value="${record.out_quantity || ''}" min="0" step="0.001" onchange="handleEditOutQuantityChange(${record.id}, this.value)" oninput="enforceQuantityMutex(this.closest('tr').querySelector('input[onchange*=\\'in_quantity\\']'), this)" ${(parseFloat(record.in_quantity) || 0) > 0 ? 'disabled' : ''}>` :
