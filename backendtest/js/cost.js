...
async function updateCharts(data) {
    ...
    // 步骤2：预渲染静态图
    const staticConfig = JSON.parse(JSON.stringify(chartConfig));
    staticConfig.options.animation = false;

    // Check if gradients were lost during JSON stringify/parse (they will be)
    // We need to re-apply gradients or accept plain colors for the static flash.
    // Actually, gradients are functions or canvas objects, they WILL be lost.
    // We should probably just use the dynamic config, but disable animation on it for the first pass.
    // However, ChartJS modifies the config object.
    // Let's manually disable animation on a shallow copy of options.

    // BETTER APPROACH:
    // 1. Create static config manually or reuse
    // 2. Since gradients are functions/objects, JSON.parse(JSON.stringify) is DESTRUCTIVE.
    // We must NOT use JSON deep copy for Chart.js configs with functions/gradients.

    // Instead, let's just modify the options of the 'chartConfig' for the first render,
    // then modify them back for the second.

    // Temporarily disable animation
    const originalAnimation = chartConfig.options.animation;
    chartConfig.options.animation = false;

    costChart = new Chart(ctx1, chartConfig);
    ...
    // Restore animation and set new values
