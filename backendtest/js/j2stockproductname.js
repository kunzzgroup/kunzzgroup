...
function switchView(viewType) {
    if (viewType === 'list') {
        // 跳转到总库存页面
        window.location.href = 'j2stocklist';
    } else if (viewType === 'records') {
        // 跳转到进出货页面
        window.location.href = 'j2stockedit';
...
async function loadStockData() {
    ...
    try {
        ...
        params.append('system_assign', SYSTEM_TYPE);  // 自动过滤J2系统数据
...
function addNewRow() {
    ...
    const newData = {
        ...
        supplier: '',
        applicant: '',
        system_assign: SYSTEM_TYPE,  // 自动设置为J2系统
        approver: ''
    };
