...
const CURRENT_USER_APPLICANT = window.CURRENT_USER_APPLICANT || '';
...
async function applyPagePermissions() {
    try {
        const res = await fetch('../hr/generatecodeapi.php', {
            method: 'POST',
        ...
        row.innerHTML = `
                    ...
                    <td>
                        <select class="excel-select ${!isNewRow ? 'readonly' : ''}" data-field="category" data-row="${rowId}" 
                            ...
                            <option value="Service Line" ${data.category === 'Service Line' ? 'selected' : ''}>Service Line</option>
