...
async function initSidebar() {
    try {
        const response = await fetch(`${BASE_PATH}api/sidebar_data.php`);
...
function renderMenu(perms, user, urls) {
    ...
    html += `
        <div class="informationmenu-footer">
            <button class="logout-btn" onclick="location.href='${BASE_PATH}includes/logout.php'">
                登出
            </button>
        </div>
    `;
...
function renderBrandItems(perms) {
    ...
    if (p.kunzz_holdings) {
        html += createMenuItemWithSubmenu('KUNZZ HOLDINGS SDN BHD', 'KUNZZ HOLDINGS SDN BHD',
            `<a href="${BASE_PATH}brand/corporate_blueprint.php" class="submenu-item">企业蓝图</a>`
        );
    }
    ...
    if (p.tokyo_cuisine) {
        ...
        if (p.j1) {
            subContent += createExpandableSubmenuItem('j1-options', 'J1 (MIDVALLEY)',
                (p.j1_schedule ? `<a href="${BASE_PATH}hr/schedule_manager.php?restaurant=J1" class="sub-option">员工排班表</a>
                                  <a href="${BASE_PATH}hr/phone_manage.php?restaurant=J1" class="sub-option">员工手机记录</a>` : '')
            );
        }
        if (p.j2) {
            subContent += createExpandableSubmenuItem('j2-options', 'J2 (PARADIGM MALL)',
                (p.j2_schedule ? `<a href="${BASE_PATH}hr/schedule_manager.php?restaurant=J2" class="sub-option">员工排班表</a>
                                  <a href="${BASE_PATH}hr/phone_manage.php?restaurant=J2" class="sub-option">员工手机记录</a>` : '')
            );
        }
        if (subContent) {
    ...
    if (p.tokyo_izakaya) {
        ...
        if (p.j3) {
            subContent += createExpandableSubmenuItem('j3-options', 'J3 (DESA TEBRAU)',
                (p.j3_schedule ? `<a href="${BASE_PATH}hr/schedule_manager.php?restaurant=J3" class="sub-option">员工排班表</a>
                                  <a href="${BASE_PATH}hr/phone_manage.php?restaurant=J3" class="sub-option">员工手机记录</a>` : '')
            );
        }
        if (subContent) {
...
function renderAnalyticsItems(perms, urls) {
    ...
    if (p.kpi_report) html += createSimpleMenuItem(`${BASE_PATH}analytics/kpi.php`, 'KPI报表');
...
function renderHRItems(perms) {
    ...
    if (p.staff_management) {
        html += createSimpleMenuItem(`${BASE_PATH}hr/generatecode.php`, '职员管理');
        html += createSimpleMenuItem(`${BASE_PATH}hr/qna.php`, '问卷回答');
        html += createSimpleMenuItem(`${BASE_PATH}hr/evaluation_form.php`, '考核表单');
    }
    return html;
}
...
function renderResourceItems(perms) {
    ...
    if (p.stock_inventory) {
        ...
        html += `<div class="menu-item-wrapper">
                    <a href="${BASE_PATH}resource/stocklistall.php" class="informationmenu-item" onclick="redirectToAllowedStockPage(event)">
                        库存
                    </a>
                 </div>`;
    }
    if (p.dishware) html += createSimpleMenuItem(`${BASE_PATH}resource/dishware_stock.php`, '碗碟');
    if (p.price_comparison) html += createSimpleMenuItem(`${BASE_PATH}resource/price.php`, '价格对比');
    return html;
}
...
function renderVisualItems(perms) {
    ...
    html += createSimpleMenuItem(`${BASE_PATH}visual/bgmusicupload.php`, '背景音乐');

    html += createMenuItemWithSubmenu('首页', '首页',
        `<a href="${BASE_PATH}visual/homepage1upload.php" class="submenu-item">第一页</a>`
    );

    html += createMenuItemWithSubmenu('关于我们', '关于我们',
        `<a href="${BASE_PATH}visual/aboutpage1upload.php" class="submenu-item">第一页</a>
         <a href="${BASE_PATH}visual/aboutpage4upload.php" class="submenu-item">第四页</a>`
    );

    html += createMenuItemWithSubmenu('旗下品牌', '旗下品牌',
        `<a href="${BASE_PATH}visual/tokyopage1upload.php" class="submenu-item">第一页</a>
         <a href="${BASE_PATH}visual/tokyopage5upload.php" class="submenu-item">第五页</a>`
    );

    html += createMenuItemWithSubmenu('加入我们', '加入我们',
        `<a href="${BASE_PATH}visual/joinpage1upload.php" class="submenu-item">第一页</a>
         <a href="${BASE_PATH}visual/joinpage2upload.php" class="submenu-item">第二页</a>
         <a href="${BASE_PATH}visual/joinpage3upload.php" class="submenu-item">第三页</a>`
    );

    html += createSimpleMenuItem(`${BASE_PATH}visual/corporate_blueprint_edit.php`, '企业蓝图管理');
...
async function redirectToAllowedStockPage(event) {
    ...
    try {
        const res = await fetch(`${BASE_PATH}hr/generatecodeapi.php`, {
            method: 'POST',
        ...
        window.location.href = `${BASE_PATH}resource/stocklistall.php?system=${targetSystem}`;
    } catch (e) {
        window.location.href = `${BASE_PATH}resource/stocklistall.php?system=central`;
    }
}
