...
export default function JoinJobs({ onJobClick }) {
  ...
  const { companies, loading, error, deptOrder, deptDisplay } = useJobs('en');
  ...
  return (
    <div className={`job-section${isMobile ? ' job-section--mobile-pure' : ''}`}>
      ...
      {error ? <div className="no-jobs">Failed to load positions. Please check the server.</div> : null}

      {!loading && !error && isMobile ? (
        <>
          <div className="jobs-company-tabs" role="tablist" aria-label="Select company">
