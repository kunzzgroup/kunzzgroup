...
import {
  fetchAboutpage4UploadFragment,
  useAboutpage4UploadLegacyBoot,
} from '../hooks/useAboutpage4UploadLegacyBoot.js';

export default function Aboutpage4UploadPage() {
  ...
  const { loading: bootLoading, error: bootError } = useAboutpage4UploadLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchAboutpage4UploadFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="aboutpage4upload.css" className="aboutpage4-upload-shell">
      {showLoading && (
      ...
      {markup && (
        <>
          <div ref={contentRef} className="backend-php-content" />
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化发展历史管理...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
