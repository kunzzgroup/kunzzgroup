...
import {
  fetchBgMusicUploadFragment,
  useBgMusicUploadLegacyBoot,
} from '../hooks/useBgMusicUploadLegacyBoot.js';

export default function BgMusicUploadPage() {
  ...
  const { loading: bootLoading, error: bootError } = useBgMusicUploadLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchBgMusicUploadFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="bgmusicupload.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化背景音乐管理...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
