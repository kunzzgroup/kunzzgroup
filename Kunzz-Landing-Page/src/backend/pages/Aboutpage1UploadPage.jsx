...
import {
  fetchAboutpage1UploadFragment,
  useAboutpage1UploadLegacyBoot,
} from '../hooks/useAboutpage1UploadLegacyBoot.js';

export default function Aboutpage1UploadPage() {
  ...
  const { loading: bootLoading, error: bootError } = useAboutpage1UploadLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchAboutpage1UploadFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="aboutpage1upload.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化关于我们媒体管理...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
