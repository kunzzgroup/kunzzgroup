...
import {
  fetchSupplyFragment,
  useSupplyLegacyBoot,
} from '../hooks/useSupplyLegacyBoot.js';
...
export default function SupplyPage() {
  ...
  const { loading: bootLoading, error: bootError } = useSupplyLegacyBoot(searchReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchSupplyFragment();
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  useLayoutEffect(() => {
    ...
    mountLegacySmartSearch(host, {
      ...
      placeholder: '输入材料名称',
      onChange: () => {
        if (typeof window.refreshSupplySearch === 'function') {
          window.refreshSupplySearch();
        }
      },
    });
    ...
    >
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化供应商价格对比数据...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
