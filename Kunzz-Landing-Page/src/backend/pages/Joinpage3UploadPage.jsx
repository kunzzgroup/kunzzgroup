...
import {
  fetchJoinpage3UploadFragment,
  useJoinpage3UploadLegacyBoot,
} from '../hooks/useJoinpage3UploadLegacyBoot.js';

export default function Joinpage3UploadPage() {
  ...
  const { loading: bootLoading, error: bootError } = useJoinpage3UploadLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchJoinpage3UploadFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="joinpage3upload.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化招聘职位管理...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
