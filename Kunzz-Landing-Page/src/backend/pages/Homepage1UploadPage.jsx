...
import {
  fetchHomepage1UploadFragment,
  useHomepage1UploadLegacyBoot,
} from '../hooks/useHomepage1UploadLegacyBoot.js';

export default function Homepage1UploadPage() {
  ...
  const { loading: bootLoading, error: bootError } = useHomepage1UploadLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchHomepage1UploadFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="homepage1upload.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化首页媒体管理...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
