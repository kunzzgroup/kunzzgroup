...
import { fetchQnaFragment, useQnaLegacyBoot } from '../hooks/useQnaLegacyBoot.js';
...
export default function QnaPage() {
  ...
  const { loading: bootLoading, error: bootError } = useQnaLegacyBoot(markupReady, contentRef);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchQnaFragment();
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="qna.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化问卷页面...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
