...
import {
  fetchCorporateBlueprintFragment,
  useCorporateBlueprintLegacyBoot,
} from '../hooks/useCorporateBlueprintLegacyBoot.js';

export default function CorporateBlueprintPage() {
  ...
  const { loading: bootLoading, error: bootError } = useCorporateBlueprintLegacyBoot(
    markupReady,
    contentRef,
  );
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchCorporateBlueprintFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="corporate_blueprint.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化企业蓝图...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
