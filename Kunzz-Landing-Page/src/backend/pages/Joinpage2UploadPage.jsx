...
import {
  fetchJoinpage2UploadFragment,
  useJoinpage2UploadLegacyBoot,
} from '../hooks/useJoinpage2UploadLegacyBoot.js';

export default function Joinpage2UploadPage() {
  ...
  const { loading: bootLoading, error: bootError } = useJoinpage2UploadLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchJoinpage2UploadFragment(fragmentQuery);
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="joinpage2upload.css">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化足迹照片管理...</p>
            </div>
          )}
          {bootError && (
            <div className="container">
