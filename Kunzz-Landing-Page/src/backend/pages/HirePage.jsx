...
import { fetchHireFragment, useHireLegacyBoot } from '../hooks/useHireLegacyBoot.js';
...
export default function HirePage() {
  ...
  const { loading: bootLoading, error: bootError } = useHireLegacyBoot(markupReady);
  ...
  useEffect(() => {
    ...
    async function loadMarkup() {
      ...
      try {
        const html = await fetchHireFragment();
        if (!cancelled) {
          setMarkup(html);
        }
      } catch (error) {
  ...
  return (
    <BackendLayout stylesheet="hire.css" className="hire-page-shell">
      {showLoading && (
      ...
      {markup && (
        <>
          ...
          {showBootSpinner && (
            <div className="layout-container">
              <p style={{ padding: '0 24px 24px', color: '#6b7280' }}>正在初始化招聘列表...</p>
            </div>
          )}
          {bootError && (
            <div className="layout-container">
