...
function CostCard({ icon, label, value, className = '' }) {
  return (
    <div className={`card${className ? ` ${className}` : ''}`}>
      <div className="card-body">
        <div className="cost-card-vertical">
          <div className="icon dynamic-color">{icon}</div>
          <div>
...
export default function CostCards({ summary, restaurant, hasRestaurant }) {
  if (!hasRestaurant) {
    return (
      <div className="cost-grid" id="cost-grid">
        <div className="card">
          <div className="card-body">
            <p className="cost-label">请先选择餐厅以查看成本指标</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="cost-grid" id="cost-grid">
      ...
      <CostCard
        ...
        label="成本率"
        value={formatPercent(summary.costPercent)}
      />
      <CostCard
        icon={<i className="fas fa-box" />}
        label="库存（最后）"
        value={formatCurrency(summary.lastStock)}
      />
      <CostCard
        icon={<i className="fas fa-warehouse" />}
        label="库存（现在）"
        value={formatCurrency(summary.currentStock)}
      />
      {isJ1 && (
        <>
          <CostCard
            ...
            label="供应→J2 (RM)"
            value={formatCurrency(summary.j2Supply)}
          />
          <CostCard
            className="supply-card"
            icon={<i className="fas fa-arrow-right" />}
            label="供应→J3 (RM)"
            value={formatCurrency(summary.j3Supply)}
          />
        </>
      )}
      <CostCard
        icon={<i className="fas fa-shuttle-van" />}
        label="Grab Food 成本 (RM)"
        value={formatCurrency(summary.grabCost)}
      />
      <CostCard
        icon={
          <svg
        ...
        label="Foodpanda 成本 (RM)"
        value={formatCurrency(summary.foodpandaCost)}
      />
      <CostCard
        icon={<i className="fas fa-shopping-bag" />}
        label="Shopee Food 成本 (RM)"
        value={formatCurrency(summary.shopeeCost)}
      />
    </div>
  );
}
